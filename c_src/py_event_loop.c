...
cleanup_native:
    ...
    /* Clean up task queue (call_soon_threadsafe) */
...
ERL_NIF_TERM nif_event_loop_new(ErlNifEnv *env, int argc,
    ...
    /* Initialize task queue for call_soon_threadsafe */
...
ERL_NIF_TERM nif_event_loop_run_async(ErlNifEnv *env, int argc,
    ...
    /* Get the event loop via policy to ensure we get the ErlangEventLoop */
    PyObject *get_loop = NULL;

    /* Try get_event_loop_policy().get_event_loop() first */
    PyObject *policy = PyObject_CallMethod(asyncio, "get_event_loop_policy", NULL);
    if (policy != NULL) {
        get_loop = PyObject_CallMethod(policy, "get_event_loop", NULL);
        Py_DECREF(policy);
    }

    /* Fallback to asyncio.get_event_loop() */
    if (get_loop == NULL) {
        PyErr_Clear();
        get_loop = PyObject_CallMethod(asyncio, "get_event_loop", NULL);
    }

    /* Last resort: get_running_loop() */
    if (get_loop == NULL) {
        PyErr_Clear();
    ...
    /* Wake up the event loop to process the new task */
    if (PyObject_HasAttrString(get_loop, "_wakeup")) {
        PyObject *wakeup_result = PyObject_CallMethod(get_loop, "_wakeup", NULL);
        Py_XDECREF(wakeup_result);
    }
...
/* ============================================================================
 * Thread-Safe Task Submission (call_soon_threadsafe pattern)
 * ============================================================================ */

/**
 * @brief Atom for task_ready wakeup message
 */
static ERL_NIF_TERM ATOM_TASK_READY = 0;

/**
 * call_soon_threadsafe(LoopRef, CallerPid, Ref, Module, Func, Args, Kwargs) -> ok | {error, Reason}
 *
 * Thread-safe task submission that works from any dirty scheduler thread.
 * Uses enif_iovq for atomic queue operations and enif_send for wakeup.
 */
ERL_NIF_TERM nif_call_soon_threadsafe(ErlNifEnv *env, int argc,
    ...
    if (!loop->task_queue_initialized || loop->task_queue == NULL) {
    ...
    /* Build a tuple with all task info for serialization:
     * {caller_pid, ref, module, func, args, kwargs} */
    ERL_NIF_TERM task_info = enif_make_tuple6(
        env,
        argv[1],  /* CallerPid */
        argv[2],  /* Ref */
        argv[3],  /* Module */
    ...
    /* Serialize task info to binary */
    ErlNifBinary task_bin;
    if (!enif_term_to_binary(env, task_info, &task_bin)) {
    ...
    /* Create an iovec from the binary */
    ErlNifBinary *heap_bin = enif_alloc(sizeof(ErlNifBinary));
    if (heap_bin == NULL) {
    ...
    *heap_bin = task_bin;
    ...
    SysIOVec iov[1];
    iov[0].iov_base = heap_bin->data;
    iov[0].iov_len = heap_bin->size;

    ErlNifIOVec eiov;
    eiov.size = heap_bin->size;
    eiov.iovcnt = 1;
    eiov.iov = iov;
    eiov.ref_bins = (void **)&heap_bin;

    if (!enif_ioq_enqv(loop->task_queue, &eiov, 0)) {
        pthread_mutex_unlock(&loop->task_queue_mutex);
        enif_release_binary(&task_bin);
        enif_free(heap_bin);
    ...
    /* Send wakeup to worker (enif_send is thread-safe) */
    if (loop->has_worker) {
        /* Initialize atom if needed */
        if (ATOM_TASK_READY == 0) {
            ATOM_TASK_READY = enif_make_atom(env, "task_ready");
        }
 ...
 *
 * Process all pending tasks from the queue. Called by the event worker
 * when it receives a task_ready message.
 */
ERL_NIF_TERM nif_process_ready_tasks(ErlNifEnv *env, int argc,
    ...
    /* Check if task queue is initialized */
    if (!loop->task_queue_initialized || loop->task_queue == NULL) {
        return ATOM_OK;  /* Nothing to process */
    }

    /* Check queue size */
    pthread_mutex_lock(&loop->task_queue_mutex);
    size_t queue_size = enif_ioq_size(loop->task_queue);
    pthread_mutex_unlock(&loop->task_queue_mutex);
    ...
    PyObject *policy = PyObject_CallMethod(asyncio, "get_event_loop_policy", NULL);
    if (policy != NULL) {
        event_loop = PyObject_CallMethod(policy, "get_event_loop", NULL);
        Py_DECREF(policy);
    }
    if (event_loop == NULL) {
        PyErr_Clear();
        event_loop = PyObject_CallMethod(asyncio, "get_event_loop", NULL);
    }
    if (event_loop == NULL) {
        PyErr_Clear();
        Py_DECREF(run_and_send);
    ...
    const int max_tasks_per_batch = 100;  /* Prevent starvation */

    while (tasks_processed < max_tasks_per_batch) {
        /* Dequeue one task */
        pthread_mutex_lock(&loop->task_queue_mutex);

        size_t remaining = enif_ioq_size(loop->task_queue);
        if (remaining == 0) {
            pthread_mutex_unlock(&loop->task_queue_mutex);
            break;
        }
        ...
        /* Peek the data */
        int iovcnt = 0;
        SysIOVec *iov = enif_ioq_peek(loop->task_queue, &iovcnt);
        if (iov == NULL || iovcnt == 0) {
            pthread_mutex_unlock(&loop->task_queue_mutex);
            break;
        }

        /* Copy data before dequeue */
        size_t data_size = iov[0].iov_len;
        unsigned char *data = enif_alloc(data_size);
        if (data == NULL) {
            pthread_mutex_unlock(&loop->task_queue_mutex);
            break;
        }
        memcpy(data, iov[0].iov_base, data_size);

        /* Dequeue the bytes we just copied */
        size_t dequeued = 0;
        enif_ioq_deq(loop->task_queue, data_size, &dequeued);

        pthread_mutex_unlock(&loop->task_queue_mutex);

        /* Deserialize task info */
        ERL_NIF_TERM task_term;
        ErlNifEnv *task_env = enif_alloc_env();
        if (task_env == NULL) {
            enif_free(data);
            continue;
        }

        if (!enif_binary_to_term(task_env, data, data_size, &task_term,
                                  ERL_NIF_BIN2TERM_SAFE)) {
            enif_free(data);
            enif_free_env(task_env);
            continue;
        }
        enif_free(data);

        /* Extract tuple: {CallerPid, Ref, Module, Func, Args, Kwargs} */
        int arity;
        const ERL_NIF_TERM *elements;
        if (!enif_get_tuple(task_env, task_term, &arity, &elements) || arity != 6) {
            enif_free_env(task_env);
            continue;
        }

        ErlNifPid caller_pid;
        if (!enif_get_local_pid(task_env, elements[0], &caller_pid)) {
            enif_free_env(task_env);
            continue;
        }

        ERL_NIF_TERM ref_term = elements[1];

        ErlNifBinary module_bin, func_bin;
        if (!enif_inspect_binary(task_env, elements[2], &module_bin) ||
            !enif_inspect_binary(task_env, elements[3], &func_bin)) {
            enif_free_env(task_env);
            continue;
        }
        ...
        /* Convert args list to Python tuple */
        unsigned int args_len;
        if (!enif_get_list_length(task_env, elements[4], &args_len)) {
            Py_DECREF(func);
            enif_free_env(task_env);
            continue;
        }
        ...
        PyObject *args = PyTuple_New(args_len);
        ERL_NIF_TERM head, tail = elements[4];
        bool args_ok = true;
        for (unsigned int i = 0; i < args_len && args_ok; i++) {
            enif_get_list_cell(task_env, tail, &head, &tail);
            PyObject *arg = term_to_py(task_env, head);
            if (arg == NULL) {
                args_ok = false;
                PyErr_Clear();
            } else {
        ...
        /* Convert kwargs */
        PyObject *kwargs = NULL;
        if (enif_is_map(task_env, elements[5])) {
            kwargs = term_to_py(task_env, elements[5]);
        }
        ...
        if (!is_coro) {
            ...
            if (erlang_mod != NULL) {
                ...
                if (pid_obj != NULL) {
                    ...
                    PyObject *py_ref = term_to_py(task_env, ref_term);
                    if (py_ref != NULL) {
                        PyObject *ok_tuple = PyTuple_Pack(2,
                            PyUnicode_FromString("ok"), coro);
                        PyObject *msg = PyTuple_Pack(3,
                            PyUnicode_FromString("async_result"),
                            py_ref, ok_tuple);

                        PyObject *send_result = PyObject_CallMethod(
                            erlang_mod, "send", "OO",
                            (PyObject *)pid_obj, msg);
                        Py_XDECREF(send_result);
                        Py_DECREF(msg);
        ...
        /* Create wrapped coroutine: _run_and_send(coro, caller_pid, ref) */
        PyObject *wrapped_coro = PyObject_CallFunction(
            run_and_send, "OOO",
            coro, (PyObject *)pid_obj, py_ref);
        ...
        PyObject *task = PyObject_CallMethod(event_loop, "create_task", "O", wrapped_coro);
        Py_DECREF(wrapped_coro);
    ...
    /* Run pending callbacks on the event loop */
    if (PyObject_HasAttrString(event_loop, "_run_once")) {
        PyObject *run_result = PyObject_CallMethod(event_loop, "_run_once", NULL);
