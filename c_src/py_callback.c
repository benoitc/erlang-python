...
static PyObject *erlang_call_impl(PyObject *self, PyObject *args) {
    ...
    if (!tl_allow_suspension) {
        ...
        /* Callback handler and pipe of the context */
        if (!has_context_handler) {
            Py_DECREF(call_args);
            enif_free_env(msg_env);
            PyErr_SetString(PyExc_RuntimeError, "erlang.call: no callback handler for this context");
            return NULL;
        }
        ErlNifPid *handler_pid = &tl_current_context->callback_handler;
        int read_fd = tl_current_context->callback_pipe[0];
